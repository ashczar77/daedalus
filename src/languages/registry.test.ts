...
describe('languages registry', () => {
  ...
  it('looks up lessons and filters by path', () => {
    ...
    expect(lessonsForPath('java-advanced').length).toBe(10)
    expect(lessonsForPath('spring-core').length).toBe(7)
    expect(getLanguageLesson('java-gc-generations')?.order).toBe(2)
    expect(getLanguageLesson('java-gc-tuning')?.order).toBe(3)
    expect(getLanguageLesson('java-static')?.order).toBe(4)
    expect(getLanguageLesson('spring-beans-gc')?.order).toBe(3)
  })
})
